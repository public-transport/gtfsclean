	...
	for id, t := range feed.Trips {
		...
		for i := 1; i < len(t.StopTimes); i++ {
			...
			if dist >= 7000 {
				...
				if gtfs.GetTypeFromExtended(t.Route.Type) == 1 && speed > 200 {
					feed.DeleteTrip(id)
					break
				}
				...
				if gtfs.GetTypeFromExtended(t.Route.Type) == 3 && speed > 200 {
					feed.DeleteTrip(id)
					break
				}
				...
				if gtfs.GetTypeFromExtended(t.Route.Type) == 11 && speed > 100 {
					feed.DeleteTrip(id)
					break
				}
